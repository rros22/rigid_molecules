...
void h2o_buffer::allocate(unsigned n)
{
  ...
  else
  {
    ...
    size_t buffer_size  = n*(sizeof(site_positions) + 3*sizeof(double) +
    ...
    site_forces_x = (double *)(site_pos + 4*n); //4 sites per molecule
    site_forces_y = site_forces_x + n;
    site_forces_z = site_forces_y + n;
    x_lin_dyn = (lin_dyn_x *)(site_forces_z + n);
