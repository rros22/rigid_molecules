...
void h2o_buffer::allocate(unsigned n)
{
  ...
  size_t buffer_size = n*(sizeof(water_site_positions) + sizeof(water_site_forces) +
                          3*sizeof(lin_dyn_x) + sizeof(quaternion_orientation) + sizeof(quaternion_derivatives)
                           + sizeof(torques));
  //allocate
  ...
  orientations = (quaternion_orientation *)(z_lin_dyn + n);
...
void h2o_buffer::initialise(double xyz[6])
{
  ...
  for (int i = 0; i < side; i++)
  {
    for (int j = 0; j < side; j++)
    {
      ...
      //orientation
      orientations[i + side*(j + side*k)].current.set_quaternion(angle, axis);
      }
    }
  }
...
void h2o_buffer::site_global_coordiantes()
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    //O
    orientations[i].current.transform_vector(O_L, offset, global_coordinates);
    ...
    //H1
    orientations[i].current.transform_vector(H1_L, offset, global_coordinates);
    ...
    //H2
    orientations[i].current.transform_vector(H2_L, offset, global_coordinates);
    ...
    //q1
    orientations[i].current.transform_vector(q1_L, offset, global_coordinates);
...
void h2o_buffer::next_site_global_coordinates()
{
  //site local coordinates
  double O_L[3] = {0, -0.065555, 0};
  double H1_L[3] = {-0.9572*sin(52.26*M_PI/180), 0.9572*cos(52.26*M_PI/180) - 0.065555, 0};
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    //O
    orientations[i].next.transform_vector(O_L, offset, global_coordinates);
    water_site_pos[i].O.x = global_coordinates[0];
    water_site_pos[i].O.y = global_coordinates[1];
    water_site_pos[i].O.z = global_coordinates[2];
    //H1
    orientations[i].next.transform_vector(H1_L, offset, global_coordinates);
    water_site_pos[i].H1.x = global_coordinates[0];
    water_site_pos[i].H1.y = global_coordinates[1];
    water_site_pos[i].H1.z = global_coordinates[2];
    //H2
    orientations[i].next.transform_vector(H2_L, offset, global_coordinates);
    water_site_pos[i].H2.x = global_coordinates[0];
    water_site_pos[i].H2.y = global_coordinates[1];
    water_site_pos[i].H2.z = global_coordinates[2];
    //q1
    orientations[i].next.transform_vector(q1_L, offset, global_coordinates);
    water_site_pos[i].q1.x = global_coordinates[0];
    water_site_pos[i].q1.y = global_coordinates[1];
    water_site_pos[i].q1.z = global_coordinates[2];
  }
...
void h2o_buffer::debug()
{
  ...
  orientations[0].current.set_quaternion(angle, axis);
  orientations[1].current.set_quaternion(angle_2, axis_2);
